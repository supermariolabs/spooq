...
class Engine(conf: ApplicationConfiguration) {
    ...
    job.steps.foreach(step => {
      ...
      Try {
        ...
        if (knownKinds.contains(step.kind)) {
          ...
          this.getClass.getMethod(s"process${Utils.functionByKind(step.kind)}", classOf[Step],classOf[SparkSession]).invoke(this, step, spark).asInstanceOf[Boolean]
        }
        else false
      } match {
        case Success(exitStatus) => {
          if (exitStatus) logger.info(s"Elapsed ${(System.nanoTime()-t0)/1000000000} second(s)")
          report.put(step.id, Report(index, exitStatus,None))
        }
        case Failure(exception) => {
          ...
          report.put(step.id, Report(index, false,Some(reason)))
        }
      }
    })
  ...
  def processInputStream(in: Step)(implicit spark: SparkSession): Boolean = {
    ...
    in.avro.foreach(avroDefined => {
      val avroSchema = in.avroSchema.getOrElse("")
  ...
  def processVariable(trx: Step)(implicit spark: SparkSession): Boolean = {
    ...
    if (collect.size==1) {
      logger.info("Single variable... (variables.put(trx.id,collect(0)))")
      ...
      logger.info("Collection... (variables.put(trx.id,collect))")
